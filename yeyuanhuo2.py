...
def tiaozhan():
    btnx = 0
    btny = 0
    testbutton = pag.locateOnScreen('image/yytiaozhan.png')
    if testbutton != None:
...
def kaishizhandou():
    btnx = 0
    btny = 0
    testbutton = pag.locateOnScreen('image/yytiaozhan.png',confidence=confi)
    if testbutton != None:
        btnx, btny = pag.center(testbutton)
    return btnx, btny
...
def yuhun(iter):
    ...
    while (counter < iter):
        # 开始查找挑战
        btnx, btny = iterFind("kaishizhandou", beginInterval=beginfightInterval, iternum=endIter)
        if btnx == -1 or btny == 0:
            print("未找到开始战斗，程序结束")
            return False
