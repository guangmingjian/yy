...
def yuhun(iter):
    ...
    while (counter < iter):
        # 开始查找挑战
        btnx, btny = BM.iterfindpic(imgroot + "zudui" + imgtype, confi=0.8, internaltime=0.4, mosttime=endIter)
        ...
        BM.jiancha(imgroot + "zudui" + imgtype,0.8,btnx, btny,0.4)
        time.sleep(3)

        btnx,btny = BM.iterfindpic(imgroot+ "zidongpipei"+imgtype,confi=0.8,internaltime=0.001,mosttime=endIter)
        time.sleep(0.2)
        pag.click(btnx,btny,duration=0.001)
        BM.jiancha(imgroot + "zidongpipei" + imgtype, 0.8, btnx, btny, 0.4)


        # BM.jianchari(imgroot+ "lvjiantou"+imgtype,0.8,343,508,internal=0.1)
        time.sleep(3)
        btnx, btny = BM.iterfindpic(imgroot + "zhunbei" + imgtype, confi=0.8, internaltime=0.001, mosttime=endIter)
        time.sleep(0.2)
        pag.click(btnx, btny-20, duration=0.001)
        BM.jiancha(imgroot + "zhunbei" + imgtype, 0.8, btnx, btny-20, 0.4)
        ...
        #
        # btnx = 1077 + random.randint(-maxrand - 30, maxrand + 30)
        ...
        # pag.click(btnx, btny,duration=0.2)
        #
        # BM.jiancha(imgroot + "dianjijixu" + imgtype, 0.8, btnx, btny, 0.2)
        # print("点击结束")
        # counter += 1
        # print("*********************第%d次"%counter)
