...
def yuhun(iter):
    ...
    while (counter < iter):
        # 开始查找挑战
        # btnx, btny = BM.iterfindpic(imgroot + "tiaozhan" + imgtype, confi=0.8, internaltime=0.4, mosttime=endIter)
        ...
        # pag.click(duration=0.2)
        #
        # BM.jiancha(imgroot + "tiaozhan" + imgtype,0.8,btnx, btny,0.4)
        # time.sleep(3)

        btnx,btny = BM.iterfindpic(imgroot+ "dajiutouxiang"+imgtype,confi=0.8,internaltime=0.001,mosttime=endIter)
        time.sleep(0.2)
        pag.click(860,620 + random.randint(-10,10),duration=0.001)
        ...
        #
        # # 查找结束标志
        # btnx, btny = BM.iterfindpic(imgroot+ "dianjijixu"+imgtype,confi=0.8,internaltime=0.001,mosttime=endIter)
        #
